...
@interface MFSideMenuContainerViewController ()
{
    ...
    UIView *backView;
    SideMenuTableViewController *cont;
}
...
@property (nonatomic, assign) CGFloat prevSelectedViewX;
...
//        NSString *onlineUsers = [defaults valueForKey:@"onlineUsers"];
        ...
        {
            cont = (SideMenuTableViewController *) self.leftMenuViewController;
            cont.onlineUsers.text =  [[Utility sharedInstance] convertNumberIntoDepiction:[Utility sharedInstance].onlineUsers];
        }
...
- (void)openLeftSideMenuCompletion:(void (^)(void))completion {
    ...
    backView = [[UIView alloc] initWithFrame:appDelegate.window.frame];
    [backView setBackgroundColor:[UIColor colorWithRed:0 green:0 blue:0 alpha:0.2]];
    backView.tag = 101;
    self.prevSelectedViewX = backView.frame.origin.x;
    [UIView animateWithDuration:0.2
                          delay:0
                        options:UIViewAnimationOptionBeginFromCurrentState
                     animations:^{
                         if ([self.centerViewController isKindOfClass:[UINavigationController class]]) {
                             UINavigationController *navCont = (UINavigationController *)self.centerViewController;
                             [navCont.view addSubview:backView];
                         }
                     }
...
//own method call.
//        [self setNavigationBarViewTo:menuState];
...
#pragma mark- navigationbarViewAddition method
-(void) setNavigationBarViewTo:(MFSideMenuState)menuState
{
       switch (menuState)
    {
        case MFSideMenuStateLeftMenuOpen:
          
        {
            ...
            CGFloat initialXPosition = -(appdel.window.frame.size.width);
            UIButton *cancelButton;
            if ([Utility sharedInstance].isDeviceIpad) {
                navigationBarImageView = [[UIImageView alloc]initWithFrame:CGRectMake(initialXPosition, 0,appdel.window.frame.size.width,85)];
                cancelButton = [[UIButton alloc]initWithFrame:CGRectMake(navigationBarImageView.frame.size.width - 70, 15, 55, 55)];
            }
            else
            {
                navigationBarImageView = [[UIImageView alloc]initWithFrame:CGRectMake(initialXPosition, 0,appdel.window.frame.size.width,64)];
                cancelButton = [[UIButton alloc]initWithFrame:CGRectMake(navigationBarImageView.frame.size.width - 43, navigationBarImageView.frame.size.height/2 - 33/2, 33, 33)];
            }
            navigationBarImageView.image = [UIImage imageNamed:@"bg_navigation"];
            navigationBarImageView.contentMode = UIViewContentModeTop;
            navigationBarImageView.clipsToBounds = YES;

            [cancelButton setImage:[UIImage imageNamed:@"books_cross"] forState:UIControlStateNormal];
            [navigationBarImageView addSubview:cancelButton];
            [cancelButton addTarget:self action:@selector(cancelActionButton
) forControlEvents:UIControlEventTouchUpInside];
            [navigationBarImageView addSubview:cancelButton];

            [self.view addSubview:navigationBarImageView];
//changing position with animation in sync with opening of leftSideMenu.
            CGFloat duration = [self animationDurationFromStartPosition:initialXPosition toEndPosition:0];
            [UIView animateWithDuration:duration animations:^{
                CGRect frame = navigationBarImageView.frame;
                frame.origin.x = 0;
                navigationBarImageView.frame = frame;
            }];
//            [UIView animateWithDuration:duration animations:^{
//                [self setCenterViewControllerOffset:offset];
//                           } completion:^(BOOL finished) {
//                innerCompletion();
            ...
            break;
        case MFSideMenuStateClosed:
            [navigationBarImageView removeFromSuperview];
        default:
            break;
    }
}
-(void)cancelActionButton
{
    [self setMenuState:MFSideMenuStateClosed completion:NULL];
}
@end